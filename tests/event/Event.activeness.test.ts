...
describe("Event isActive behavior", () => {
	...
	beforeEach(() => {
		...
		switchEvent = source<number>();
		state1 = state<number>(0, switchEvent);
	});

	describe("initial state", () => {
		it("should be inactive when no effects, states, or child events", () => {
			expect(source1.isActive).toBe(false);
		});
	});
	...
	describe("effects (on method)", () => {
		...
		it("should remain active with multiple effects", () => {
			const [, dispose1] = source1.on(() => {});
			const [, dispose2] = source1.on(() => {});
		...
		it("should call activate/deactivate hooks correctly with effects", () => {
			const activateSpy = vi.spyOn(source1 as any, "activate");
			const deactivateSpy = vi.spyOn(source1 as any, "deactivate");
	...
	describe("depended states (writeOn method)", () => {
		it("should become active when writing to a state", () => {
			...
			const dispose = source1.writeOn(state1);
		...
		it("should remain active with multiple depended states", () => {
			const state2 = state(10, switchEvent);

			const dispose1 = source1.writeOn(state1);
			const dispose2 = source1.writeOn(state2);
	...
	describe("combined conditions", () => {
		it("should remain active when multiple conditions are met", () => {
			...
			const stateDispose = source1.writeOn(state1);
			expect(source1.isActive).toBe(true);
	...
	describe("edge cases", () => {
		it("should handle removing non-existent effects gracefully", () => {
			const callback = () => {};
			const [, dispose] = source1.on(callback);
