...
describe("MergedEvent", () => {
	...
	describe("caching behavior", () => {
		it("should cache emitted value and not recompute on multiple getEmittedValue calls", () => {
			...
			const firstCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length;
			expect(firstResult?.()).toEqual({ type: "both", left: "test", right: 42 });

			// Second call should use cached value, not recompute
			const secondResult = mergedEvent.getEmittedValue();
			const secondCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length;
			expect(secondResult?.()).toEqual({ type: "both", left: "test", right: 42 });
		...
		it("should clear cache after commit and recompute on next getEmittedValue", () => {
			...
			const firstResult = mergedEvent.getEmittedValue();
			const firstCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length;
			expect(firstResult?.()).toEqual({ type: "both", left: "first", right: 1 });
			...
			const secondResult = mergedEvent.getEmittedValue();
			const secondCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length;
			expect(secondResult?.()).toEqual({ type: "both", left: "second", right: 2 });
		...
		it("should clear maybeEmittedValue in commit method", () => {
			...
			const firstResult = mergedEvent.getEmittedValue();
			expect(firstResult?.()).toEqual({ type: "both", left: "cached", right: 100 });
			...
			expect(afterCommitResult?.()).toEqual({ type: "both", left: "cached", right: 100 }); // Same value
		...
		it("should cache independently for nested MergedEvents", () => {
			...
			const firstCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length + thirdGetEmittedValue.mock.calls.length + innerGetEmittedValue.mock.calls.length;
			...
			const secondCallCount = leftGetEmittedValue.mock.calls.length + rightGetEmittedValue.mock.calls.length + thirdGetEmittedValue.mock.calls.length + innerGetEmittedValue.mock.calls.length;
			...
			expect(intermediate1?.()).toEqual({ type: "both", left: "nested", right: 42 });
