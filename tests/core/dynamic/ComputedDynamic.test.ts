...
describe("ComputedDynamic", () => {
	...
	it("should track dependencies correctly with nested reads", () => {
		...
		// Verify dependencies
		expect(outerComputed.lastRead?.dependencies?.includes(innerComputed)).toBe(
			true,
		);
		expect(outerComputed.lastRead?.dependencies?.includes(state2)).toBe(true);
		expect(innerComputed.lastRead?.dependencies?.includes(state1)).toBe(true);
